...
published hours cells appear to describe. `ECBTC` must select
`globex_event_contracts_btc` in every era: it rode the shared grid from
2023-03-12 to 2026-05-28, and from 2026-05-29 that key serves a 24/7 grid on
the sourced intersection of the two CME statements of its weekday close, which
disagree by an hour. The same applies to the
...
Spot-Quoted Futures, CME Group event contracts, Event Contracts on Bitcoin Futures, and Eurex fixed income all ship as
sourced keys.
