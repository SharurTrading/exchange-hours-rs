...
- **Product-family calendars** — all 29 operator-derived `MarketHoursKey`
...
`MarketHoursKey` has 30 variants—29 operator-derived product-family keys plus
...
19:00-21:00 CT on 2018-01-21 and no longer wraps past midnight. The mini-sized
grains (`XC`/`XK`/`XW`/`MKC`) must likewise select `globex_mini_grains`: their
day session closed 30 minutes after the standard grid from before the
January-2010 floor until the grids converged on 2022-10-02, and they skipped
the standard grid's 2015-07-05 close change entirely. CME weather
...
`globex_equity_index` nor `globex_cryptocurrency` answers for them. CME Group
event contracts (`ECES`/`ECNQ`/`ECRTY`/`ECYM`/`EC6E`/`ECCL`/`ECNG`/`ECGC`/`ECSI`/`ECHG`
and the hourly Chapter 23A roots) must select `globex_event_contracts`: their
per-root Termination of Trading times are stated *expiration* times, not session
closes, so all of them run one 17:00→16:00 CT grid rather than the six their
published hours cells appear to describe. `ECBTC` rides that key only from
2023-03-12 to 2026-05-28, because CME moved that one root to 24/7 trading on
2026-05-29 and two of its own primary sources disagree by an hour on the new
daily close, so no key models it afterwards. The same applies to the
...
Spot-Quoted Futures, CME Group event contracts, and Eurex fixed income all ship as
sourced keys.
...
to 15 executable** across the 49 rows in the ledger. The order-entry majority is
the exact *day* an older queue or post-close phase started, with the trading
session itself sourced. The executable fifteen — the ICE Futures U.S. keys, CME
...
2026-09-02, and `globex_event_contracts`, whose daily close is carried back to its
2022 launch — are each served conservatively, erring toward closed rather than
...
**Hours verified at the review date for each product family:** `29 of 29` operator-derived
...
rows are **Primary** and 23 are **Partial**, because a named historical
