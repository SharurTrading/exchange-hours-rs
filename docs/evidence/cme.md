...
**Abstention and audited-normal silence are both states the audit reads.** A family with no window on a date abstains and the families that cover it decide: `globex_livestock` has no 2016-2018 table, so the other five decide that era's 36 dates — 9 closures and 27 withheld. Inside a window a family that states no row has audited the date normal, which is not the same state as silence: a date one family states and another audited normal is a disagreement and ships `Unsourced`. Of the 226 rows withheld, **220 are disagreements of that kind and 6 are dates every routed family itself marks not worked up** — 2019-06-19, 2020-06-19, 2021-06-19, 2023-01-16, 2023-02-20 and 2023-04-07 — which are the families' own marker rather than a dispute. **One reading this audit depends on is not yet settled.** The count above treats an `Unsourced` routed family as abstaining on a date the others state, which is how 2024-03-29 and 2024-12-25 ship `Closed` with seven families stating a closure and `globex_nikkei_225_dollar` marking the date. Two other shapes in this table read an `Unsourced` family as a disagreement instead: 2025-01-02 (`globex_grains` late open, the rest silent) ships `Unsourced`, and 2024-12-31 (Nikkei `Unsourced`, the rest silent) ships nothing. No 2013-2015 date has the shape, so this wave neither introduced nor encoded one, and the decision is tracked as **#111**.
