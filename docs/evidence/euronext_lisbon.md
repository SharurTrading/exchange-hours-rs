...
- 2023-03-20 — T1 — Euronext Go-Live Weekend Guidelines — legacy-market pre-opening moves from 07:15 to 07:30 CET, that is 06:15 to 06:30 Lisbon local time.
...
- **Scope.** The profile represents the principal continuous-trading share segment, not every Euronext Lisbon instrument or segment. Lisbon retains its own IANA zone, `Europe::Lisbon`, while the Central-European clock is translated one civil hour earlier locally, so no fixed UTC shift is applied.
...
- **Horizon, residual risk.** The earliest artifact cited for the legacy grid is the operator's December-2010 special-day appendix; the January-2010 to December-2010 interval is asserted on the source set's "January-2010-or-launch" status rather than on an artifact dated inside it. Closing condition: a Euronext trading appendix or notice dated in or before January 2010 that prints the legacy 07:15/09:00/17:30/17:40 grid. If none is retrieved, the horizon should be restated as the special-day appendix's own first attested day, 2010-12-24.
...
> [`euronext_paris`](euronext_paris.md), [`euronext_amsterdam`](euronext_amsterdam.md), [`euronext_brussels`](euronext_brussels.md), [`euronext_milan`](euronext_milan.md).
> The anchor identity for this module is [`euronext_paris`](euronext_paris.md), which receives the
