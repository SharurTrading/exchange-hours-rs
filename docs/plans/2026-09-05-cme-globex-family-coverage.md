...
### 5. The 37-shape research queue — **DELIVERED**, then superseded

> 2026-09-12 (UTC): the architectural review and the charter's LAW-SERVICE-TIERS put
> trade-type variants out of scope until a consumer maps one; see
> [2026-09-12-architectural-review.md](2026-09-12-architectural-review.md).
Researched and adversarially challenged; the deliverable was
[the trade-type handoff](2026-09-05-cme-trade-type-handoff.md), which carried the
root-to-key table, the unresolved list with the document each needed, and the decision
rule the survey produced. Eight research gates and nine adversarial verdicts then ran
against it and produced [the trade-type key plan](2026-09-12-cme-trade-type-keys.md),
which sequenced 13 PRs and 32 keys. Of that sequence only the fence PR (#78) and the
five metals TAS keys (#83, now dormant identities) landed before the charter superseded
it; both plan documents are historical records now, and no further trade-type key is
authored until a consumer maps a variant.
