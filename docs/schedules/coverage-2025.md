...
are unchanged. The `eurex` row's `Holidays`, `2025+ dates`, `Missing / disputed` and `Complete?`
cells were corrected on 2026-09-26 UTC when its 2025 holiday rows shipped, together with §1's
scope count and the `eurex` endpoint row and table-shape bullet in
[Artifact resolution](#artifact-resolution).
are unchanged. The `cme` and `cbot` rows' own prose counts, and §2's, were corrected on
2026-09-26 UTC to the 48 and 47 their `Unsrc 2025+ dates` cells and the inventory fence derive, and
...
cells name what remains open rather than the merges.
...
`iceus` ships a 2026-01-01..2028-01-03 window, so it answers no holiday question over any part
of 2025. `cfe` and `eurex` shipped the same 2026-only window until their 2025 rows landed in
separate changes on 2026-09-26 UTC; both now audit 2025-01-01..2026-12-31. `iceus` ships only four
scheduling rows: its
...
`iceus` is the one served scope whose evidence file still lacks the fixed `### Documents` shape
that #98 tracks; `cfe` and `eurex` acquired that shape with their 2025 rows, so #98's remaining
...
`globex_cryptocurrency` loses the 24/7-era 16:00-16:01 CT minute to an unstateable gap and carries an
undated five-day-era Pre-Open onset. `comex` and `nymex` are intersections too, but each
