...
`eurex` ships a single 2026 window and `iceus` a 2026-01-01..2028-01-03 window, so both answer no
holiday question over any part of 2025. `cfe` shipped the same 2026-only window until its 2025 rows
landed; it now audits 2025-01-01..2026-12-31. `iceus` ships only four scheduling rows: its
...
real instrument takes. Stage 4's PR order (section 8, items 3-5) already anticipates them, and
`eurex` and `iceus` are the two served scopes whose evidence files still lack the fixed
`### Documents` shape that #98 tracks; `cfe` acquired that shape with its 2025 rows, so #98's
remaining scope is the dormant CFE and Eurex keys and the six ICE Futures U.S. family files.
...
`globex_cryptocurrency` publishes special sessions no row states and carries an undated five-day-era
Pre-Open onset. `comex` and `nymex` are intersections too, but each
...
| `cfe` | `CBOE-SU-2025-NEW-YEAR` | T1 | `holidays/raw/cfe-2010-2025/live/2024__CFE-Modified-Trading-Hours-for-the-New-Year-s-Day-Holiday.pdf` - sha256 reproduces as `9bbaa3bb3f8484356a756bd3bd04677450d6215ff5e96c0e514a63a4aac3e7d7` | `CBOE-HOURS-USFUT-2026` | T1 | `holidays/raw/cfe-eurex-ice-cde-smfe-2026-2027/cboe_hours_us_futures.html` and `cboe_us_futures_holidays_2026.csv` - sha256 reproduces as `b1b38ba66a87…` and `cc98e21e97b8…`; it is the horizon document because Cboe has published no 2027 schedule |
...
All thirty endpoints of the fifteen scopes that carry a `### Documents` table reproduce.
The two that show no endpoint are a shape gap, not a missing source:

- `eurex` and `iceus` carry **no Markdown table with a `sha256` header at all**; their
  digests (`EUREX-HOLREG-2026`, `IFUS-CAL-2026`, `IFUS-CAL-2027` and the
  ...
  and they are also two of the two scopes that ship no 2025 holiday coverage. Their digests are
  **not machine-verified by this stage**; closing that needs the sweep rule widened to
  prose-quoted digests, and it is recorded as unverified rather than verified.

`cfe` acquired the fixed shape with its 2025 rows, so its baseline and horizon endpoints now
resolve through it: the baseline is the 2025 New Year's notice, the first row of its 2025 window,
and the horizon is the `2026 Futures Holiday Schedule` page, because Cboe has published nothing
for 2027. Both digests were recomputed by this stage and reproduce. The baseline being a 2025
holiday notice rather than the earlier `CBOE-HOURS-USFUT-2026` page is the substantive change:
before it, `cfe`'s only resolved artifact was the horizon, so the sweep had nothing to compare
the two ends against.
