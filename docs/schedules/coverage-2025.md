...
are unchanged. `globex_cryptocurrency`'s `2025+ dates` count, `Missing / disputed` and `Complete?`
cells moved again on 2026-09-26 UTC, when its nine merged trade dates shipped as rows: the count is
now 32 and the two cells name what remains open rather than the merges.
