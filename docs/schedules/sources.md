...
- **Status:** each row is narrowly scoped to ordinary individual-stock options' current generic 07:30 order-acceptance queue plus 09:30–16:00 regular session. ETF, ETN, index, FLEX, floor-only, and venue-designated product sessions are excluded. C1 has a pre-floor primary RTH baseline; C2, BZX Options, and EDGX Options are closed before exact sourced launches. The exact day each generic queue began remains a documented Partial history gap. The 2026-08-31 queue review established why: on every US options venue the generic order-acceptance start is an operator system setting published on a mutable hours/system-settings page, not a rulebook boundary carrying a filed operative date, so the onset days are knowledge-bound rather than unsearched. The two filings above put the C2 and BZX queues at 07:30 no later than 2019-05-10 and 2020-02-04 respectively.
