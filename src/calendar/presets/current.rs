...
pub fn hours_for_exchange(exch: Exchange) -> MarketHours {
    match exch {
        ...
        // profile.
        Exchange::Cme => session_profile(MarketHoursKey::GlobexEquityIndex).to_market_hours(CalendarSource::Exchange(exch)),
        Exchange::Cbot => session_profile(MarketHoursKey::GlobexGrains).to_market_hours(CalendarSource::Exchange(exch)),
        Exchange::Comex | Exchange::Nymex => from_profile(exch, &futures_us::ENERGY_METALS_CURRENT),
        Exchange::Eurex => session_profile(MarketHoursKey::Eurex).to_market_hours(CalendarSource::Exchange(exch)),
        ...
        Exchange::Sgx => session_profile(MarketHoursKey::Sgx).to_market_hours(CalendarSource::Exchange(exch)),
        Exchange::Cfe => session_profile(MarketHoursKey::CfeVix).to_market_hours(CalendarSource::Exchange(exch)),
