...
// Cocoa runs one same-day executable session; the ICE master hours table
// carries no footnote marker on its row, so nothing commences on the previous
// calendar evening. The 20:00 pre-open is modelled Monday-Thursday because a
// wrapping rule cannot carry a Friday evening across the weekend.
// Narrative: docs/evidence/ice_us_cocoa.md
...
// pre-open running to the next morning's open. Cocoa publishes no tradeable
// phase outside its executable session, so `extended` is empty.
// Narrative: docs/evidence/ice_us_cocoa.md
...
// Baseline before 2014-02-03: open 04:00 NY, close 14:00 NY, printed outright
