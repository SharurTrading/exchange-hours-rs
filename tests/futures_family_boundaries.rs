...
fn sgx_equity_index_t_plus_one_opens_fifteen_minutes_earlier_from_2025_04_07() {
    ...
    for (key, probe, moved) in [
        ...
        assert!(
            !hours_for_market_hours_key(key, last_earlier_day).is_open(probe),
            "{key:?}: the profile in force through 2025-04-06 still opens T+1 at the \
             earlier-era time ({moved})"
        );
        assert!(
            hours_for_market_hours_key(key, first_revised_day).is_open(probe),
            "{key:?}: the profile in force from 2025-04-07 opens T+1 fifteen minutes \
             earlier ({moved})"
        );
    }
