...
fn no_late_open_ships_and_the_post_closure_reopen_is_the_normal_open() {
    ...
    // the counts pin the shape of all four eras, so a row on a date no test
    // names fails here.
    ...
    assert_eq!(
        ...
        (31, 96, 3, 3),
        "closed, early-close, late-open and unsourced rows over all four windows"
    );
}
...
fn the_coverage_window_bounds_what_the_table_answers() {
    ...
    // 2019-2021 interval sits between the 2016-2018 and 2022-2024 waves, so
    // 2024-12-31 — this probe before the 2022-2024 wave shipped — is now
    // inside a window and is an audited normal date.
    assert!(!coverage.contains(day(2020, 12, 31)));
    assert_eq!(calendar.holiday_on(day(2020, 12, 31)), None);
    assert_eq!(calendar.holiday_on(day(2020, 12, 25)), None);
    assert!(calendar.is_open(ct((2020, 12, 25), (9, 0, 0))));
