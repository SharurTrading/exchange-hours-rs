...
/// 100 closures, 23 early closes, the four 2010-2012 late opens and the six
/// `Unsourced` statements, nothing else.
#[test]
fn the_table_ships_no_late_open_in_this_window() {
    ...
    assert_eq!(closed, 100, "closed rows over every audited window");
    assert_eq!(early, 23, "early-close rows over every audited window");
...
fn assert_unsourced_changes_nothing(date: NaiveDate, row: Holiday, tier: EvidenceTier) {
    ...
    // flat grid has no session on whether or not a row exists, so the strict
    // claims are made where the family could trade and the equality claims
    // where it could not.
    if matches!(date.weekday(), Weekday::Sat | Weekday::Sun) {
        ...
        assert_eq!(
            calendar.is_closed_all_day_on(date, SessionKind::Both),
            detached.is_closed_all_day_on(date, SessionKind::Both),
            "{date}"
        );
    } else {
        assert!(
...
#[test]
fn era_2022_2024_window_sits_third_and_the_2013_2018_interval_is_unaudited() {
    let calendar = calendar();
    let coverage = calendar
        .holiday_coverage()
        .expect("globex_livestock ships a table");
    ...
    for date in [(2013, 1, 1), (2015, 12, 25), (2017, 6, 14)] {
...
/// a closure must take the whole civil day.
#[test]
