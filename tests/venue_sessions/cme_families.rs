...
fn cme_weekday_preopen_changed_on_the_sourced_2010_day() {
    for key in [
        ...
        let before = hours_for_market_hours_key_as_of(key, ct((2010, 11, 14), (12, 0, 0)));
        let after = hours_for_market_hours_key_as_of(key, ct((2010, 11, 15), (0, 0, 0)));
...
fn grains_keep_exact_sourced_order_phase_revisions() {
    let before_pcp = hours_for_market_hours_key_as_of(
        MarketHoursKey::GlobexGrains,
        ct((2010, 4, 18), (12, 0, 0)),
    );
    let expanded_pcp = hours_for_market_hours_key_as_of(
        MarketHoursKey::GlobexGrains,
        ct((2010, 4, 19), (0, 0, 0)),
    );
    ...
    let before_morning_change = hours_for_market_hours_key_as_of(
        MarketHoursKey::GlobexGrains,
        ct((2011, 12, 26), (12, 0, 0)),
    );
    let after_morning_change = hours_for_market_hours_key_as_of(
        MarketHoursKey::GlobexGrains,
        ct((2011, 12, 27), (0, 0, 0)),
    );
    ...
    let from_2013_queue = hours_for_market_hours_key_as_of(
        MarketHoursKey::GlobexGrains,
        ct((2013, 8, 18), (0, 0, 0)),
    );
...
fn dated_cme_calendars_expose_the_undated_phase_limit_without_inventing_cutovers() {
    ...
    let grains_fixed = hours_for_market_hours_key(MarketHoursKey::GlobexGrains);
...
fn cryptocurrency_current_profile_preserves_exact_open_state() {
    ...
    let hours = hours_for_market_hours_key(MarketHoursKey::GlobexCryptocurrency);
...
fn cryptocurrency_history_covers_launch_24_7_and_temporary_maintenance() {
    ...
    let transition_day = hours_for_market_hours_key_as_of(
        MarketHoursKey::GlobexCryptocurrency,
        ct((2026, 5, 29), (23, 59, 59)),
    );
    let recurring_week = hours_for_market_hours_key_as_of(
        MarketHoursKey::GlobexCryptocurrency,
        ct((2026, 5, 30), (0, 0, 0)),
    );
    ...
    let normal_before_temporary = hours_for_market_hours_key_as_of(
        MarketHoursKey::GlobexCryptocurrency,
    ...
    let temporary_before_restoration = hours_for_market_hours_key_as_of(
        MarketHoursKey::GlobexCryptocurrency,
