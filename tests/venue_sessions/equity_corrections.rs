...
fn nyse_national_dormancy_and_relaunch_use_sourced_dates() {
    ...
    let dormant = hours_for_exchange(Exchange::NyseNational, cessation);
    ...
    let reopened = hours_for_exchange(Exchange::NyseNational, relaunch);
...
fn edgx_exact_2021_queue_changes_are_date_aware() {
    let before = hours_for_exchange(Exchange::CboeEdgx, et((2021, 3, 5), (12, 0, 0)));
    assert!(!before.is_open(et((2021, 3, 5), (5, 0, 0))));
    assert!(before.is_open_extended(et((2021, 3, 5), (7, 30, 0))));

    let after = hours_for_exchange(Exchange::CboeEdgx, et((2021, 3, 8), (12, 0, 0)));
    assert!(!after.is_open(et((2021, 3, 8), (3, 29, 59))));
    assert!(after.is_order_entry_only(et((2021, 3, 8), (3, 30, 0))));

    let september = hours_for_exchange(Exchange::CboeEdgx, et((2021, 9, 7), (12, 0, 0)));
...
fn edgx_unconfirmed_overnight_session_is_not_encoded() {
    ...
    let future = hours_for_exchange(Exchange::CboeEdgx, et((2026, 12, 7), (12, 0, 0)));
...
fn bzx_0230_queue_started_with_the_2025_early_session_expansion() {
    ...
    let before = hours_for_exchange(Exchange::CboeBzx, et((2025, 4, 30), (12, 0, 0)));
    assert!(!before.is_open(et((2025, 4, 30), (5, 0, 0))));
    assert!(before.is_order_entry_only(et((2025, 4, 30), (6, 0, 0))));

    let after = hours_for_exchange(Exchange::CboeBzx, et((2025, 5, 1), (12, 0, 0)));
...
fn memx_is_closed_before_its_2020_09_21_launch() {
    ...
    let before = hours_for_exchange(Exchange::MemxEq, cutover - chrono::Duration::seconds(1));
    let launched = hours_for_exchange(Exchange::MemxEq, cutover);
...
fn memx_0400_premarket_started_2025_05_19() {
    ...
    let before = hours_for_exchange(Exchange::MemxEq, cutover - chrono::Duration::seconds(1));
    let after = hours_for_exchange(Exchange::MemxEq, cutover);
...
fn memx_postmarket_close_changed_in_2020_and_2023() {
    ...
    let launch_grid = hours_for_exchange(Exchange::MemxEq, et((2020, 10, 2), (12, 0, 0)));
    let short_grid = hours_for_exchange(Exchange::MemxEq, et((2020, 10, 5), (0, 0, 0)));
    let restored = hours_for_exchange(Exchange::MemxEq, et((2023, 2, 1), (0, 0, 0)));
...
fn miax_pearl_is_closed_before_its_2020_09_29_launch() {
    ...
    let launched = hours_for_exchange(Exchange::MiaxPearlEq, cutover);
...
fn miax_pearl_extended_sessions_started_2025_02_20() {
    ...
    let after = hours_for_exchange(Exchange::MiaxPearlEq, cutover);
...
fn xetra_extended_retail_has_a_sourced_current_cutover() {
    ...
    let before = hours_for_exchange(Exchange::Xetra, cet((2025, 11, 28), (12, 0, 0)));
    ...
    let after = hours_for_exchange(Exchange::Xetra, cet((2025, 12, 1), (0, 0, 0)));
