...
fn iceus_fang_profile_is_closed_before_its_sourced_launch() {
    ...
    let closed =
        hours_for_exchange_as_of(Exchange::Iceus, launch_eve - chrono::Duration::seconds(1));
...
fn ice_canada_canola_2012_close_extension_and_2013_restoration() {
    let before = hours_for_exchange_as_of(Exchange::IceCanada, ct((2012, 6, 23), (12, 0, 0)));
    assert!(!before.is_open(ct((2012, 6, 25), (13, 15, 0))));

    let longer = hours_for_exchange_as_of(Exchange::IceCanada, ct((2012, 6, 24), (12, 0, 0)));
    assert!(longer.is_open_regular(ct((2012, 6, 25), (13, 59, 59))));
    assert!(!longer.is_open(ct((2012, 6, 25), (14, 0, 0))));

    let restored = hours_for_exchange_as_of(Exchange::IceCanada, ct((2013, 4, 7), (12, 0, 0)));
...
fn ice_canada_canola_2016_close_extension_and_2018_transfer() {
    let before = hours_for_exchange_as_of(Exchange::IceCanada, ct((2016, 1, 23), (12, 0, 0)));
    assert!(!before.is_open(ct((2016, 1, 25), (13, 15, 0))));

    let extended = hours_for_exchange_as_of(Exchange::IceCanada, ct((2016, 1, 24), (12, 0, 0)));
    ...
    let legacy = hours_for_exchange_as_of(
        Exchange::IceCanada,
        transfer_opening - chrono::Duration::seconds(1),
    );
    assert!(legacy.is_open_regular(ct((2018, 7, 27), (13, 19, 59))));

    let closed = hours_for_exchange_as_of(Exchange::IceCanada, transfer_opening);
    ...
    let current = hours_for_exchange(Exchange::IceCanada);
