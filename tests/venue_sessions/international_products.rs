...
fn ice_ftse_100_migration_and_open_extensions() {
    ...
    assert!(one.is_order_entry_only(lon((2015, 10, 1), (0, 45, 0))));
...
fn ice_endex_ttf_extension_and_reference_clock() {
    ...
    assert!(before.is_order_entry_only(zoned(Europe::Amsterdam, (2026, 4, 10), (7, 45, 0))));
    ...
    assert!(aligned.is_order_entry_only(zoned(Europe::Amsterdam, (2026, 4, 14), (1, 40, 0))));
    ...
    assert!(mismatch.is_order_entry_only(zoned(Europe::Amsterdam, (2026, 10, 27), (0, 40, 0))));
    ...
    assert!(spring_mismatch.is_order_entry_only(zoned(
        Europe::Amsterdam,
        (2027, 3, 16),
        (0, 40, 0)
    )));
...
fn ice_abu_dhabi_murban_launch_and_us_reference_clock() {
    ...
    assert!(after.is_order_entry_only(zoned(Asia::Dubai, (2021, 3, 29), (1, 0, 0))));
    ...
    assert!(winter.is_order_entry_only(zoned(Asia::Dubai, (2026, 1, 19), (2, 0, 0))));
