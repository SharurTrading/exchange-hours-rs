...
schedules, **T2** for the 15 read from its `trading-hours-by-product`
service.
...
family's ordinary week over this wave is one wrapped leg per trade date, 17:00 CT the previous evening into a 16:00 CT close on the trade date, read from `cme_group.rs CME_PROFILE_DATED_CURRENT (2021-06-27, CME Globex notice 20210621)`.
...
| 2022-05-30 | early close | `12:00 CT` | `2022-memorial-day-holiday-schedule.xls @2022-07-04T06:54:38Z` | T1 | CME prints `12:00 CT` as this date's own final close; the session that opened `17:00 CT` the previous evening is clipped there, and the crate's trade date is the local date that close falls on — the operator's event date. |
...
| 2024-01-15 | early close | `12:00 CT` | `CME-SVC-2024-01-14` | T2 | CME prints `12:00 CT` as this date's own final close; the session that opened `17:00 CT` the previous evening is clipped there, and the crate's trade date is the local date that close falls on — the operator's event date. |
| 2024-02-19 | early close | `12:00 CT` | `CME-SVC-2024-02-18` | T2 | CME prints `12:00 CT` as this date's own final close; the session that opened `17:00 CT` the previous evening is clipped there, and the crate's trade date is the local date that close falls on — the operator's event date. |
...
| 2024-05-27 | early close | `12:00 CT` | `CME-SVC-2024-05-26` | T2 | CME prints `12:00 CT` as this date's own final close; the session that opened `17:00 CT` the previous evening is clipped there, and the crate's trade date is the local date that close falls on — the operator's event date. |
| 2024-06-19 | early close | `12:00 CT` | `CME-SVC-2024-06-18` | T2 | CME prints `12:00 CT` as this date's own final close; the session that opened `17:00 CT` the previous evening is clipped there, and the crate's trade date is the local date that close falls on — the operator's event date. |
| 2024-07-03 | early close | `12:15 CT` | `CME-SVC-2024-07-03` | T2 | CME prints `12:15 CT` as this date's own final close; the session that opened `17:00 CT` the previous evening is clipped there, and the crate's trade date is the local date that close falls on — the operator's event date. |
| 2024-07-04 | early close | `12:00 CT` | `CME-SVC-2024-07-03` | T2 | CME prints `12:00 CT` as this date's own final close; the session that opened `17:00 CT` the previous evening is clipped there, and the crate's trade date is the local date that close falls on — the operator's event date. |
| 2024-09-02 | early close | `12:00 CT` | `CME-SVC-2024-09-01` | T2 | CME prints `12:00 CT` as this date's own final close; the session that opened `17:00 CT` the previous evening is clipped there, and the crate's trade date is the local date that close falls on — the operator's event date. |
| 2024-11-28 | early close | `12:00 CT` | `CME-SVC-2024-11-27` | T2 | CME prints `12:00 CT` as this date's own final close; the session that opened `17:00 CT` the previous evening is clipped there, and the crate's trade date is the local date that close falls on — the operator's event date. |
| 2024-11-29 | early close | `12:15 CT` | `CME-SVC-2024-11-27` | T2 | CME prints `12:15 CT` as this date's own final close; the session that opened `17:00 CT` the previous evening is clipped there, and the crate's trade date is the local date that close falls on — the operator's event date. |
| 2024-12-24 | early close | `12:15 CT` | `CME-SVC-2024-12-24` | T2 | CME prints `12:15 CT` as this date's own final close; the session that opened `17:00 CT` the previous evening is clipped there, and the crate's trade date is the local date that close falls on — the operator's event date. |
...
- open: printed open 17:00 CT is the family's ordinary first open — 2022-01-17, 2022-02-21, 2022-05-30, 2022-06-20, 2022-07-04, 2022-09-05, 2022-11-24, 2023-05-29, 2023-06-19, 2023-07-03, 2023-07-04, 2023-09-04, 2023-11-23, 2024-01-15, 2024-02-19, 2024-05-27, 2024-06-19, 2024-07-03, 2024-07-04, 2024-09-02, 2024-11-28
...
- status normal — 2022-01-01, 2024-12-31
