...
"""Derive the four CME venue holiday tables from the six family tables with the era.

Stage-2.2 wave-5 tool (design memo D17), the wave-4 tool re-pointed at the
2013-2015 era. It parses the `holidays!`
...
#: Families that declare no 2013-2015 window, with the reason the venue table
#: must therefore abstain rather than read their silence as `audited normal`.
NO_WINDOW = {
    "globex_cryptocurrency": "no crypto product existed before 2017-12-18",
    "globex_nikkei_225_dollar": "no Nikkei line on any 2013-2015 sheet",
}
...
def insert(tables: dict[str, Table], era: tuple[dt.date, dt.date]) -> None:
    ...
    on or after the era's first year, keeping the table strictly ascending.
...
def main() -> int:
    ...
    parser.add_argument("--era", nargs=2, type=int, default=[2013, 2015])
